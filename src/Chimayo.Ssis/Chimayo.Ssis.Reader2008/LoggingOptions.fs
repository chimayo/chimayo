module Chimayo.Ssis.Reader2008.LoggingOptions
...
let readLoggingOptions (nav : NavigatorRec) : CfLoggingOptions =
    {
        filterKind = nav |> Extractions.PropertyElement.getValueOrDefault "FilterKind" ((int) >> enum<CfLogFilterKind>) CfLogFilterKind.ByExclusion
        loggingMode = nav |> Extractions.PropertyElement.getValueOrDefault "LoggingMode" ((int) >> enum<CfLogMode>) CfLogMode.UseParentSetting
        logProviders = nav |> navMap "DTS:SelectedLogProvider" readSelectedLogProvider
