module Chimayo.Ssis.CodeGen.Pipeline.Transform.RowCount
...
let buildInputConnection (DfInputConnection (DfName _, DfOutputReference (DfComponentReference cname, DfName oname))) =
    functionApplicationQ <@ RowCount.connect_input_by_name @> [ cname |> constant ; oname |> constant ] |> InlineExpression

let buildComponent (c : DfComponent) =
    let dummy = Chimayo.Ssis.Ast.DataFlowApi.RowCount.create "dummy" []
    ...
    functionApplicationQ <@ Chimayo.Ssis.Ast.DataFlowApi.RowCount.create @>
        [
            c.name |> constant
            ListExpression
                [
                    ...
                    yield! mutate (=) RowCount.get_result_variable <@ RowCount.set_result_variable @> makeScopedVariableReference
