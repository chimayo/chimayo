module Chimayo.Ssis.Reader2016.Pipeline.Source.OleDb
...
let readSource (nav : NavigatorRec) =
    ...
    let decodeParameterDirection d =
        match d with
        | "Input" -> DfInputParameter
        | "Output" -> DfOutputParameter
        | "InputOutput" -> DfInputOutputParameter
        | _ -> failwith "Unexpected parameter direction specified"
    ...
    let pm' = 
        ...
        |> Array.map (fun (spec:string) -> spec.Split(',') |> fun ars -> ars.[0].Trim(), ars.[1].Trim())
        |> Array.map (fun (a,b) -> a.Trim('"').Split(':') |> fun ars -> ars.[0].Trim(), ars.[1].Trim(), b)
        |> Array.map (fun (a,b,c) -> a |> extractParamInt, b |> decodeParameterDirection, nav |> dtsIdToVariable ((new System.Guid(c)).ToString("B")) |> optionOrDefault (CfVariableRef.fromString ""))
        |> Array.sortBy (fun (a,b,c) -> a)
        |> Seq.ofArray
