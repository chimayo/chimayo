module Chimayo.Ssis.Reader2016.ForEachLoopTask
...
let readVariableMapping nav =
    let index = nav |> Extractions.anyInt "DTS:ValueIndex" -1
    let result =
        {
            expressions = nav |> Common.readPropertyExpressions
            target = nav |> Extractions.anyString "@DTS:VariableName" "" |> CfVariableRef.fromString
        } : CfForEachLoopVariableMapping
    index,result

let readVariableMappings nav =
    nav
    |> navMap "DTS:ForEachVariableMappings/DTS:ForEachVariableMapping" readVariableMapping
    |> List.sortBy fst
    |> List.map snd
